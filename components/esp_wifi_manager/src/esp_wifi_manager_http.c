...
static esp_err_t handler_post_ap_start(httpd_req_t *req)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "AP start via HTTP failed: %s", esp_err_to_name(ret));
        if (ret == ESP_ERR_NO_MEM) {
            ESP_LOGE(TAG, "Insufficient memory. Free heap: %" PRIu32 " bytes.", esp_get_free_heap_size());
        }
        return send_error(req, 500, esp_err_to_name(ret));
    }
    return send_ok(req);
}
...
esp_err_t wifi_mgr_http_init(void)
{
    ...
    if (!g_wifi_mgr->httpd) {
        ...
        config.max_uri_handlers = 48;  // WiFi API(18) + WebUI(3) + Captive(8) + Boombox(5) + OPTIONS(6) + reserve(8)
    ...
    // Register Boombox API handlers
    ESP_LOGI(TAG, "Registering Boombox API handlers...");
    esp_err_t boombox_ret = wifi_manager_register_boombox_handlers(g_wifi_mgr->httpd);
    if (boombox_ret == ESP_OK) {
        ESP_LOGI(TAG, "Boombox API handlers registered successfully");
    } else {
        ESP_LOGE(TAG, "Failed to register Boombox API handlers: %s", esp_err_to_name(boombox_ret));
    }
