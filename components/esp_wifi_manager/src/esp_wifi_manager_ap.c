...
esp_err_t wifi_manager_start_ap(const wifi_mgr_ap_config_t *config)
{
    ...
    // Check available heap before starting AP (WiFi needs ~50KB)
    uint32_t free_heap = esp_get_free_heap_size();
    if (free_heap < 51200) {
        ESP_LOGE(TAG, "Insufficient memory to start AP: %" PRIu32 " bytes free (need 50KB)", free_heap);
    ...
    // Validate expanded SSID
    if (ssid_expanded[0] == '\0') {
        ESP_LOGE(TAG, "SSID expansion failed");
        g_wifi_mgr->ap_active = false;
        return ESP_ERR_INVALID_ARG;
    }
    
    ESP_LOGI(TAG, "Starting AP: %s (free heap: %" PRIu32 " bytes)", ssid_expanded, free_heap);

    // Only change mode if not already in APSTA
    wifi_mode_t current_mode;
    if (esp_wifi_get_mode(&current_mode) != ESP_OK) {
        ESP_LOGW(TAG, "Failed to read WiFi mode before starting AP");
    } else if (current_mode != WIFI_MODE_APSTA) {
        esp_err_t r = esp_wifi_set_mode(WIFI_MODE_APSTA);
        if (r != ESP_OK) {
            ESP_LOGE(TAG, "esp_wifi_set_mode(APSTA) failed: %s", esp_err_to_name(r));
            g_wifi_mgr->ap_active = false;
            return r;
        }
    }
    ...
    if (wifi_mgr_dns_start() != ESP_OK) {
        ESP_LOGW(TAG, "Failed to start DNS server for captive portal");
    }

    // Configure static IP
    if (ap_cfg->ip[0]) {
        if (!g_wifi_mgr->ap_netif) {
            ESP_LOGW(TAG, "AP netif is NULL, cannot configure static IP");
        } else {
            esp_err_t r = esp_netif_dhcps_stop(g_wifi_mgr->ap_netif);
            if (r != ESP_OK) {
                ESP_LOGW(TAG, "esp_netif_dhcps_stop failed: %s", esp_err_to_name(r));
            }

            esp_netif_ip_info_t ip_info = {0};
            if (inet_pton(AF_INET, ap_cfg->ip, &ip_info.ip) <= 0) {
                ESP_LOGW(TAG, "Invalid AP IP format: %s", ap_cfg->ip);
            } else {
                ...
                if (esp_netif_set_ip_info(g_wifi_mgr->ap_netif, &ip_info) != ESP_OK) {
                    ESP_LOGW(TAG, "esp_netif_set_ip_info failed");
                }
            }

            if (esp_netif_dhcps_start(g_wifi_mgr->ap_netif) != ESP_OK) {
                ESP_LOGW(TAG, "esp_netif_dhcps_start failed");
            }
        }
    }
...
void wifi_mgr_start_ap_mode(void)
{
    esp_err_t ret = wifi_manager_start_ap(NULL);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "wifi_mgr_start_ap_mode: start AP failed: %s", esp_err_to_name(ret));
        if (ret == ESP_ERR_NO_MEM) {
            ESP_LOGE(TAG, "Insufficient memory. Free heap: %" PRIu32 " bytes.", esp_get_free_heap_size());
        }
    }
}
...
esp_err_t wifi_manager_set_ap_config(const wifi_mgr_ap_config_t *config)
{
    ...
    if (g_wifi_mgr->ap_active) {
        esp_err_t ret = wifi_manager_start_ap(config);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "wifi_manager_set_ap_config: start AP failed: %s", esp_err_to_name(ret));
            if (ret == ESP_ERR_NO_MEM) {
                ESP_LOGE(TAG, "Insufficient memory. Free heap: %" PRIu32 " bytes.", esp_get_free_heap_size());
            }
        }
